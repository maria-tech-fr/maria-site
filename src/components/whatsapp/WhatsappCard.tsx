...
export default function WhatsappCard({ visible, isOffice, onReply, onLater }: Props) {
  ...
  return (
    ...
    >
      {/* Header — sur-titre DM Mono avec dot pulsant. Vert success en ligne,
          jaune accent hors ligne. (On évite volontairement le vert WhatsApp
          officiel #25D366 et on reste sur notre #3FC163.) */}
        ...
        <span className="flex flex-1 items-center gap-2 font-mono text-[10.5px] uppercase leading-4 tracking-[0.08em] text-ink-soft/80">
          <span
            ...
            className={`inline-block h-1.5 w-1.5 flex-none rounded-full ${
              isOffice
                ? 'wa-ping-dot bg-success'
                : 'wa-ping-dot-offline bg-accent'
            }`}
            style={{
              boxShadow: isOffice
                ? '0 0 0 4px rgba(63,193,99,0.20)'
                : '0 0 0 4px rgba(254,194,60,0.20)',
            }}
          />
          {status.label}
        </span>
      ...
      {/* Body — avatar (toujours jaune accent, c'est l'identité marque) +
          pastille de présence en bas-droite (vert si online, jaune si offline). */}
      <div className="flex items-start gap-3">
        ...
        >
          ...
          <span
            aria-hidden
            className={`absolute -bottom-px -right-px h-3 w-3 rounded-full border-[2.5px] border-paper ${
              isOffice ? 'bg-success' : 'bg-accent'
            }`}
          />
        </span>
