        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3 lg:gap-5">
          {references.map((r, i) => (
            <Reveal key={r.slug} delay={120 + i * 80} className="h-full">
              <Link
                href={`/besoins/${r.slug}`}
                className="group/r flex h-full flex-col gap-3 rounded-[12px] border border-paper-edge bg-paper p-7 transition-all duration-300 ease-out hover:-translate-y-0.5 hover:border-ink"
              >
