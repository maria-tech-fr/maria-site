    ...
    defineField({
      ...
      fields: [
        ...
        defineField({
          ...
          type: 'object',
          fields: [
            defineField({ name: 'surTitre', title: 'Sur-titre', type: 'string', initialValue: '// message reçu' }),
            defineField({ name: 'titre', title: 'Titre (H1)', type: 'string', initialValue: 'Merci, votre message est arrivé.' }),
            defineField({ name: 'description', title: 'Description', type: 'text', rows: 3, initialValue: 'On vous répond sous 24 h ouvrées. En attendant, vous pouvez consulter notre journal — on y partage nos points de vue sur l’IA en entreprise.' }),
          ],
        }),
      ],
    }),
